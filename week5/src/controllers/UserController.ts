...
const createUser = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!result) return res.status(statusCode.CONFLICT).send(util.fail(statusCode.CONFLICT, message.DUPLICATED_EMAIL));
...
const signInUser = async (req: Request, res: Response) => {
    const error = validationResult(req); // express-validator 에러 확인
    if(!error.isEmpty()) {
        return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, message.BAD_REQUEST));
    } 
    ...
    try {
        ...
        const data = {
            ...
            accessToken // 로그인 후 access token도 반환해줌
        }
